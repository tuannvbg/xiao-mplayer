...
class List {
    constructor(player) {
        ...
        this.showing = true
        this.player.template.list.style.height = `${Math.min(
            this.player.template.list.scrollHeight,
            this.player.options.listMaxHeight
        )}px`
    ...
    bindEvents() {
        this.player.template.list.addEventListener('click', (e) => {
            ...
            const audioIndex =
                parseInt(
                    target.getElementsByClassName('xiao-mplayer-list-index')[0]
                        .innerHTML
                ) - 1
    ...
    add(audios) {
        ...
        if (wasSingle && this.audios.length > 1) {
            this.player.container.classList.add('xiao-mplayer-withlist')
        }
        ...
        ].style.backgroundColor = audios.theme || this.player.options.theme
    ...
    remove(index) {
        ...
        if (this.audios[index]) {
            if (this.audios.length > 1) {
                const list = this.player.container.querySelectorAll(
                    '.xiao-mplayer-list li'
                )
                list[index].remove()
                ...
                for (let i = index; i < list.length; i++) {
                    list[i].getElementsByClassName(
                        'xiao-mplayer-list-index'
                    )[0].textContent = i
                }
                if (this.audios.length === 1) {
                    this.player.container.classList.remove(
                        'xiao-mplayer-withlist'
                    )
                }
    ...
    switch(index) {
        ...
        if (typeof index !== 'undefined' && this.audios[index]) {
            ...
            // set html
            this.player.template.pic.style.backgroundImage = audio.cover
                ? `url('${audio.cover}')`
                : ''
            this.player.theme(
            ...
            this.player.template.author.innerHTML = audio.artist
                ? ' - ' + audio.artist
                : ''

            const light = this.player.container.getElementsByClassName(
                'xiao-mplayer-list-light'
            )[0]
            if (light) {
                light.classList.remove('xiao-mplayer-list-light')
            }
            this.player.container
                .querySelectorAll('.xiao-mplayer-list li')
                [this.index].classList.add('xiao-mplayer-list-light')
            ...
            if (this.player.duration !== 1) {
                ...
                this.player.template.dtime.innerHTML = utils.secondToTime(
    ...
    clear() {
        ...
        this.index = 0
        this.player.container.classList.remove('xiao-mplayer-withlist')
        this.player.pause()
        this.audios = []
