...
class Lrc {
    ...
    show() {
        ...
        this.player.template.lrcWrap.classList.remove('xiao-mplayer-lrc-hide')
    }

    hide() {
        this.player.events.trigger('lrchide')
        this.player.template.lrcWrap.classList.add('xiao-mplayer-lrc-hide')
    }

    toggle() {
        if (
            this.player.template.lrcWrap.classList.contains(
                'xiao-mplayer-lrc-hide'
            )
        ) {
            this.show()
        } else {
    ...
    update(currentTime = this.player.audio.currentTime) {
        if (
            ...
            currentTime >= this.current[this.index + 1][0]
        ) {
            for (let i = 0; i < this.current.length; i++) {
                if (
                    currentTime >= this.current[i][0] &&
                    (!this.current[i + 1] ||
                        currentTime < this.current[i + 1][0])
                ) {
                    this.index = i
                    ...
                    this.container
                        .getElementsByClassName('xiao-mplayer-lrc-current')[0]
                        .classList.remove('xiao-mplayer-lrc-current')
                    this.container
                        .getElementsByTagName('p')
                        [i].classList.add('xiao-mplayer-lrc-current')
                }
            }
        }
    }
        ...
        if (!this.parsed[index]) {
            if (!this.async) {
                if (this.player.list.audios[index].lrc) {
                    this.parsed[index] = this.parse(
                        this.player.list.audios[index].lrc
                    )
                } else {
                ...
                xhr.onreadystatechange = () => {
                    if (
                        ...
                        if (
                            ...
                            this.player.notice(
                                `LRC file request fails: status ${xhr.status}`
                            )
                            this.parsed[index] = [['00:00', 'Not available']]
                        }
