...
const XIAO_MPLAYER_VERSION = '1.0.0'
const instances = []

class xiaoMPlayer {
    /**
     * xiao-mplayer constructor function
     *
    ...
    constructor(options) {
        ...
        this.container.classList.add('xiao-mplayer')
        if (this.options.lrcType && !this.options.fixed) {
            this.container.classList.add('xiao-mplayer-withlrc')
        }
        if (this.options.audio.length > 1) {
            this.container.classList.add('xiao-mplayer-withlist')
        }
        if (utils.isMobile) {
            this.container.classList.add('xiao-mplayer-mobile')
        }
        this.arrow = this.container.offsetWidth <= 300
        if (this.arrow) {
            this.container.classList.add('xiao-mplayer-arrow')
        }

        // save lrc
        if (this.options.lrcType === 2 || this.options.lrcType === true) {
            const lrcEle = this.container.getElementsByClassName(
                'xiao-mplayer-lrc-content'
            )
        ...
        if (this.options.fixed) {
            this.container.classList.add('xiao-mplayer-fixed')
        ...
        if (this.template.info.offsetWidth < 200) {
            this.template.time.classList.add('xiao-mplayer-time-narrow')
        }
            ...
            if (this.options.loop === 'none') {
                if (this.options.order === 'list') {
                    if (this.list.index < this.list.audios.length - 1) {
                        this.list.switch(
                            (this.list.index + 1) % this.list.audios.length
                        )
                        this.play()
                    } else {
                        this.list.switch(
    ...
    setAudio(audio) {
        ...
        if (
            ...
            this.options.customAudioType[type]
        ) {
            if (
    ...
    theme(
        color = this.list.audios[this.list.index].theme || this.options.theme,
    ...
    setUIPlaying() {
        if (this.paused) {
            this.paused = false
            this.template.button.classList.remove('xiao-mplayer-play')
            this.template.button.classList.add('xiao-mplayer-pause')
    ...
    setUIPaused() {
        if (!this.paused) {
            ...
            this.template.button.classList.remove('xiao-mplayer-pause')
            this.template.button.classList.add('xiao-mplayer-play')
            this.template.button.innerHTML = ''
            setTimeout(() => {
        ...
        this.container.classList.remove('xiao-mplayer-loading')
     ...
     */
    toggle() {
        if (this.template.button.classList.contains('xiao-mplayer-play')) {
            this.play()
        } else if (
            this.template.button.classList.contains('xiao-mplayer-pause')
        ) {
            this.pause()
        }
    }
    ...
    setMode(mode = 'normal') {
        this.mode = mode
        if (mode === 'mini') {
            this.container.classList.add('xiao-mplayer-narrow')
        } else if (mode === 'normal') {
            this.container.classList.remove('xiao-mplayer-narrow')
        }
    }
    ...
    static get version() {
        return XIAO_MPLAYER_VERSION
    }
}

export default xiaoMPlayer
